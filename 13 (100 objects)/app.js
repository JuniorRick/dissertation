...
function main() {
  ...
  function render(now) {
    ...
    drawScene(gl, cubeBuffer, programInfo, cubeTexture, deltaTime);
...
// Draw the scene.
//
function drawScene(gl, buffer, programInfo, texture, deltaTime) {
  ...
  for(let ii = 0; ii < N; ii++) {
    setBuffersAndAttributes(gl, buffer, programInfo);
    ...
    gl.drawElements(gl.TRIANGLES, buffer.len, type, offset);
  }
  rotate += deltaTime;
}
